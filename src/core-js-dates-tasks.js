...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const day = 86400000;
  return (endDate - startDate) / day + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (searchedDate >= startDate && searchedDate <= endDate) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  ...
  const resultDate = new Date(date).toLocaleString(
    'en-US',
    { timeZone: 'UTC' },
    options
  );
...
function getNextFridayThe13th(date) {
  ...
  for (let i = month; i < 12; i += 1) {
    const resultData = new Date(year, i, 13);
    if (resultData.getDay() === 5) {
      return resultData;
    }
  }
  return false;
